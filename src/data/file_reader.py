...
class FileReader:
    ...
    def load_movements(self, drone_id: str) -> Generator[Tuple[str, int], None, None]:
        ...
        if not os.path.exists(file_path):
            file_logger.file_reader(f"Movement file not found for drone {drone_id}", level="ERROR")
            return
    ...
    def load_sensor_data(self, file_path: Union[str, Path]) -> Generator[str, None, None]:
        ...
        if not os.path.exists(file_path):
            file_logger.file_reader(f"Sensor data file not found at {file_path}", level="ERROR")
            return
